...
COMANDOS_BASAS = {
    "!discord": "Únete {mensaje.user} a nuestro discord https://nocheprogramacion.com/discord",
    "!youtube": "Subscribiere a Canal de Youtube https://youtube.com/alswnet?sub_confirmation=1, {mensaje.user}",
}
...
class twithbot:
    ...
    def procesar_mensaje(self, mensaje_recivido):
        ...
        text_start = next((idx for idx, parte in enumerate(partes) if parte.startswith(":")), None)
        ...
        hast_star = next((idx for idx, parte in enumerate(irc_args) if parte.startswith("#")), None)
    ...
    def manejar_mensaje(self, mensaje_recivido):
        ...
        if mensaje.irc_command == "PING":
            ...
            logger.info(f"> PRIVMSG{mensaje.text}")
            if mensaje.text_command in COMANDOS_BASAS:
                self.manejar_comandos_base(mensaje, mensaje.text_command, COMANDOS_BASAS[mensaje.text_command])
